...
- [ ] Run `git diff --check`.
- [ ] Run `dqa --help` and the README `quick_audit.py` example.
...
- [ ] `CODE_OF_CONDUCT.md` is present and has a private reporting path.
- [ ] Bug report and feature request templates are present.
- [ ] Exactly one pull request template is present under `.github/`.
- [ ] License file is present.

## Release Notes

- [ ] Changelog entry is updated.
- [ ] The matching `docs/release/vX.Y.Z-release-notes.md` file is prepared.
